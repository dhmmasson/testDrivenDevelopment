...
- **Minor Version (1.Y.0):** Increased when new features are introduced. (e.g. you implement the feature -o minHumidity) or for backward-compatible improvement. (e.g. you handle American fighters in addition to Russian ones). A minor version is the result of several [Commits](../Conventional-Commits.md) at least one of them should be a ```feat: implemented feature X``` but there should be some ```test: add some tests for feature X```, ```refactor: create a class to handle Y``` and ```docs: add the documentation for the method C::X```
